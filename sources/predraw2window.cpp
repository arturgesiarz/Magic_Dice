#include "../headers/predraw2window.h"
    ...
    , globalConfig(globalConfig)
{
    ui->setupUi(this);
    setFixedSize(size());
    PreDraw2Window::showCorrectName();
}
...
void PreDraw2Window::showCorrectName()
{
    std::string actName = globalConfig->getNameOfPlayer(drewPresons);
    ui->playerName->setText(QString::fromStdString(actName));
}

void PreDraw2Window::drawing(QLabel* cube) {
    int cubeNumber = RandomEngine::randomNumber(1,6);
    PreDraw2Window::addPoints(drewPresons, cubeNumber);
    PreDraw2Window::changeCubesAfterDrawing(cubeNumber,cube);
}

void PreDraw2Window::changeCubesAfterDrawing(int cubeNumber, QLabel* cube)
{
...
void PreDraw2Window::changeStartCube(QLabel *cube)
{
    cube->setPixmap(QPixmap(":/cube/res/dice-?.png"));
}

void PreDraw2Window::addPoints(int playerID, int score)
{
    globalConfig->setScoreOfPlayer(playerID, score);
}


void PreDraw2Window::on_btnDraw_clicked(bool checked)
{
    ...
    // losowanie
        ...
        PreDraw2Window::drawing(ui->dice1);
        PreDraw2Window::drawing(ui->dice2);
