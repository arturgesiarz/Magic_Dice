#include "../headers/predraw5window.h"
    ...
    , globalConfig(globalConfig)
{
    ui->setupUi(this);
    setFixedSize(size());
    PreDraw5Window::showCorrectName();
}
...
void PreDraw5Window::showCorrectName()
{
    std::string actName = globalConfig->getNameOfPlayer(drewPresons);
    ui->playerName->setText(QString::fromStdString(actName));
}

void PreDraw5Window::drawing(QLabel* cube) {
    int cubeNumber = RandomEngine::randomNumber(1,6);
    PreDraw5Window::addPoints(drewPresons, cubeNumber);
    PreDraw5Window::changeCubesAfterDrawing(cubeNumber,cube);
}

void PreDraw5Window::changeCubesAfterDrawing(int cubeNumber, QLabel* cube)
{
...
void PreDraw5Window::changeStartCube(QLabel *cube)
{
    cube->setPixmap(QPixmap(":/cube/res/dice-?.png"));
}

void PreDraw5Window::addPoints(int playerID, int score)
{
    globalConfig->setScoreOfPlayer(playerID, score);
}


void PreDraw5Window::on_btnDraw_clicked(bool checked)
{
